...
class App extends React.Component {
	...
	searchClicked(){
		...
		for(var i = 0; i < test_users.length; i++){
			...
			test_users[i].birth_state = "#FFFFFF";
		}
		this.setState({
			search : str,
			users : test_users,
	...
	user_sort(num){
		...
		for(var i = 0; i < test_user.length; ++i){
			...
			test_user[i].birth_state = "#FFFFFF";
		}
		switch (num){
			case 0 :
				//전체 보기
